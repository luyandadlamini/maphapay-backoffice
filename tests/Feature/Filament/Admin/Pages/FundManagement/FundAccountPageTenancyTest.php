...
class FundAccountPageTenancyTest extends TestCase
{
    /**
    ...
    protected function setUp(): void
    {
        ...
        if ($this->isInMemorySqlite()) {
            $this->markTestSkipped(
                'FundAccountPage tenancy tests require MySQL (SQLite :memory: cannot share tables across connections)'
            );
        }
    ...
    private function seedTenantDirectly(string $tenantId): Tenant
    {
        ...
        if (! $central->table('tenants')->where('id', $tenantId)->exists()) {
            $central->table('tenants')->insert([
                'id'            => $tenantId,
                'name'          => 'FundAccountPage test tenant',
                'plan'          => 'default',
                'team_id'       => null,
                'trial_ends_at' => null,
                'created_at'    => now(),
    ...
    public function selecting_an_account_by_uuid_initializes_tenancy_for_that_accounts_tenant(): void
    {
        ...
        Livewire::test(FundAccountPage::class)
            ->assertSuccessful()
