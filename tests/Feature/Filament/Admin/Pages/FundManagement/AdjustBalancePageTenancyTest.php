...
class AdjustBalancePageTenancyTest extends TestCase
{
    /**
    ...
    protected function setUp(): void
    {
        ...
        if ($this->isInMemorySqlite()) {
            $this->markTestSkipped(
                'AdjustBalancePage tenancy tests require MySQL (SQLite :memory: cannot share tables across connections)'
            );
        }
    ...
    private function seedTenantDirectly(string $tenantId): Tenant
    {
        ...
        if (! $central->table('tenants')->where('id', $tenantId)->exists()) {
            $central->table('tenants')->insert([
                'id'            => $tenantId,
                'name'          => 'AdjustBalancePage test tenant',
                'plan'          => 'default',
                'team_id'       => null,
                'trial_ends_at' => null,
                'created_at'    => now(),
    ...
    #[Test]
    public function selecting_an_account_by_uuid_initializes_tenancy_for_that_accounts_tenant(): void
    {
        ...
        // The page mounts before an account is chosen — tenancy is NOT initialized yet.
        // Simulate the user typing the account UUID into the TextInput, which triggers
        // afterStateUpdated → lookupAccount → selectAccount → initializeTenancyForRecord.
        Livewire::test(AdjustBalancePage::class)
            ->assertSuccessful()
            ->call('selectAccount', $accountUuid);
    ...
    #[Test]
    public function tenancy_is_not_initialized_on_mount_before_account_is_selected(): void
    {
        $admin = User::factory()->create();
        $admin->assignRole('super-admin');
        $this->actingAs($admin);
