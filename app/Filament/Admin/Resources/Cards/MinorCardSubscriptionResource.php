...
use App\Filament\Admin\Resources\Cards\MinorCardSubscriptionResource\Pages;
use Filament\Forms;
...
class MinorCardSubscriptionResource extends Resource
{
    protected static ?string $model = CardSubscription::class;

    protected static ?string $navigationGroup = 'Cards';

    protected static ?string $navigationLabel = 'Minor Subscriptions';

    protected static ?string $slug = 'minor-card-subscriptions';

    protected static ?int $navigationSort = 18;

    /**
     * @return Builder<CardSubscription>
     */
    public static function getEloquentQuery(): Builder
    {
        /** @var Builder<CardSubscription> $query */
        $query = parent::getEloquentQuery();

        return $query->where('is_minor_subscription', true);
    }
        ...
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('payer.full_name')
                    ->label('Payer')
                    ->description('Minor Subscription')
                    ->searchable(),
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('override_approve')
                    ->label('Override Approve')
                    ->color('warning')
                    ->visible(fn (CardSubscription $record): bool => $record->status === CardSubscriptionStatus::PendingGuardianApproval
                        && $record->minorCardRequest !== null
                        && $record->minorCardRequest->status === MinorCardConstants::STATUS_PENDING_APPROVAL
                        && Gate::allows('minorOverrideApprove', $record))
                    ->form([
                        Forms\Components\Textarea::make('reason')
                            ->required()
                            ->minLength(30)
                            ->helperText('Requires ≥ 30 chars. Use only when guardian is unreachable and compliance has decided to approve manually.'),
                    ])
                    ->action(function (CardSubscription $record, array $data, MinorCardSubscriptionService $service, CardAuditService $audit): void {
                        Gate::authorize('minorOverrideApprove', $record);
                        /** @var \App\Models\User $admin */
                        $admin = auth()->user();
                        /** @var \App\Domain\Account\Models\MinorCardRequest $minorRequest */
                        $minorRequest = $record->minorCardRequest;
                        $service->approve($admin, $minorRequest, $data['reason']);

                        $audit->recordAdminAction(
                            admin: $admin,
                            action: 'minor_request.admin_override_approved',
                ...
                Tables\Actions\Action::make('change_plan')
                    ...
                    ->form([
                        Forms\Components\Select::make('new_plan_code')
                            ->label('New Plan')
                            ->options(fn () => CardPlan::where('active', true)->pluck('name', 'code'))
                            ->required(),
                        Forms\Components\Textarea::make('reason')
                            ->required()
                            ->minLength(10),
    ...
    {
        return [
            'index' => Pages\ListMinorCardSubscriptions::route('/'),
            'view'  => Pages\ViewMinorCardSubscription::route('/{record}'),
        ];
    }
}
