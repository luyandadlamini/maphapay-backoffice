...
final class StandardUnayoSettler implements ProviderSettler
{
    ...
    public function settle(string $providerRequestId, string $outcome, array $payload): void
    {
        ...
        DB::transaction(function () use ($providerRequestId, $status, $outcome, $payload): void {
            ...
            if ($row === null) {
                Log::warning('Standard Unayo callback for unknown wallet_provider_transaction', [
    ...
    private function creditUser(WalletProviderTransaction $row): void
    {
        ...
        if ($account === null) {
            Log::warning('Standard Unayo credit skipped: no account for user', [
        ...
        $this->walletOps->deposit(
            ...
            'standard-unayo-cashin:' . $row->provider_request_id,
    ...
    private function normaliseStatus(string $remote): string
    {
        ...
        return match (true) {
            in_array($u, ['SETTLED', 'SUCCESSFUL', 'COMPLETED'], true) => WalletProviderTransaction::STATUS_SUCCESSFUL,
            in_array($u, ['REVERSED', 'FAILED', 'REJECTED'], true)     => WalletProviderTransaction::STATUS_FAILED,
