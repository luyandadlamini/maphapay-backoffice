...
namespace App\Domain\Wallet\Providers\Emali;
...
final class EmaliAdapter implements WalletProviderAdapter
{
    public function __construct(
        private readonly EmaliClient $client,
    ) {
    }

    public function providerId(): string
    {
        return 'emali_eswatini_mobile';
    }
    ...
    public function link(string $identifier, string $currency): WalletLinkResult
    {
        ...
        return new WalletLinkResult(
            ...
            displayName: 'eMali ' . $msisdn,
            linkToken: base64_encode("emali:{$msisdn}"),
    ...
    public function collect(WalletMovementRequest $req): WalletMovementResult
    {
        ...
        try {
            $this->client->initiateCollection(
                $referenceId,
    ...
    public function disburse(WalletMovementRequest $req): WalletMovementResult
    {
        ...
        try {
            $this->client->initiateDisbursement(
                $referenceId,
                $this->amountMajor($req->amountMinor),
                strtoupper($req->currency),
                $req->providerAccountRef,
    ...
    {
        $payload = $this->client->getCollectionStatus($providerRequestId);
        $remote = strtoupper((string) ($payload['status'] ?? 'PENDING'));

        $status = match ($remote) {
            'SUCCESSFUL', 'SUCCESS', 'COMPLETED' => WalletMovementStatus::STATUS_SUCCESSFUL,
            'FAILED', 'REJECTED' => WalletMovementStatus::STATUS_FAILED,
    ...
    public function verifyWebhookSignature(string $rawBody, array $headers): bool
    {
        ...
        if ((bool) config('emali.verify_callback_token') !== false) {
            $expected = (string) config('emali.callback_token');
        ...
        if ((bool) config('emali.verify_hmac_signature') !== false) {
            $key = (string) config('emali.hmac_key');
