...
final class EmaliSettler implements ProviderSettler
{
    ...
    public function settle(string $providerRequestId, string $outcome, array $payload): void
    {
        ...
        DB::transaction(function () use ($providerRequestId, $status, $outcome, $payload): void {
            ...
            if ($row === null) {
                Log::warning('eMali callback for unknown wallet_provider_transaction', [
    ...
    private function creditUser(WalletProviderTransaction $row): void
    {
        ...
        if ($account === null) {
            Log::warning('eMali credit skipped: no account for user', [
        ...
        $this->walletOps->deposit(
            ...
            'emali-collect:' . $row->provider_request_id,
    ...
    private function normaliseStatus(string $remote): string
    {
        ...
        return match (true) {
            in_array($u, ['SUCCESSFUL', 'SUCCESS', 'COMPLETED'], true) => WalletProviderTransaction::STATUS_SUCCESSFUL,
            in_array($u, ['FAILED', 'REJECTED'], true)                 => WalletProviderTransaction::STATUS_FAILED,
