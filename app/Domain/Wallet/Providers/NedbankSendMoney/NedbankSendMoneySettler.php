...
final class NedbankSendMoneySettler implements ProviderSettler
{
    ...
    public function settle(string $providerRequestId, string $outcome, array $payload): void
    {
        ...
        DB::transaction(function () use ($providerRequestId, $status, $outcome, $payload): void {
            ...
            if ($row === null) {
                Log::warning('Nedbank Send Money callback for unknown wallet_provider_transaction', [
    ...
    private function creditUser(WalletProviderTransaction $row): void
    {
        ...
        if ($account === null) {
            Log::warning('Nedbank Send Money credit skipped: no account for user', [
        ...
        $this->walletOps->deposit(
            ...
            'nedbank-send-money-inbound:' . $row->provider_request_id,
    ...
    private function normaliseStatus(string $remote): string
    {
        ...
        return match (true) {
            in_array($u, ['COMPLETED', 'SUCCESSFUL', 'PAID'], true) => WalletProviderTransaction::STATUS_SUCCESSFUL,
            in_array($u, ['DECLINED', 'FAILED', 'EXPIRED'], true)   => WalletProviderTransaction::STATUS_FAILED,
