...
namespace App\Domain\Wallet\Providers\NedbankSendMoney;
...
final class NedbankSendMoneyAdapter implements WalletProviderAdapter
{
    public function __construct(
        private readonly NedbankSendMoneyClient $client,
    ) {
    }

    public function providerId(): string
    {
        return 'nedbank_send_money';
    }
    ...
    public function link(string $identifier, string $currency): WalletLinkResult
    {
        ...
        return new WalletLinkResult(
            ...
            displayName: 'Nedbank Send Money ' . $msisdn,
            linkToken: base64_encode("nedbank_send_money:{$msisdn}"),
    ...
    {
        $payload = $this->client->getInboundStatus($providerRequestId);
        $remote = strtoupper((string) ($payload['status'] ?? 'PENDING'));

        $status = match ($remote) {
            'COMPLETED', 'SUCCESSFUL', 'PAID' => WalletMovementStatus::STATUS_SUCCESSFUL,
            'DECLINED', 'FAILED', 'EXPIRED' => WalletMovementStatus::STATUS_FAILED,
    ...
    public function verifyWebhookSignature(string $rawBody, array $headers): bool
    {
        ...
        if ((bool) config('nedbank_send_money.verify_callback_token') !== false) {
            $expected = (string) config('nedbank_send_money.callback_token');
        ...
        if ((bool) config('nedbank_send_money.verify_hmac_signature') !== false) {
            $key = (string) config('nedbank_send_money.hmac_key');
