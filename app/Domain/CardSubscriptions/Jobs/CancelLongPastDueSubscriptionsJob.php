...
class CancelLongPastDueSubscriptionsJob implements ShouldQueue
{
    use Dispatchable;
        ...
        foreach (Tenant::query()->cursor() as $tenant) {
            ...
            try {
                CardSubscription::query()
                    ->where('status', CardSubscriptionStatus::Suspended)
                    ->where('suspended_at', '<=', now()->subDays(11))
                    ->orderBy('id')
                    ->chunkById(100, function ($subs) use ($subscriptions): void {
                        foreach ($subs as $sub) {
                            $subscriptions->terminateUnpaid($sub);
                        }
                    });
            } catch (Throwable $e) {
                Log::error('cards.cancel_long_past_due.tenant_failed', [
                    'tenant_id' => $tenant->id,
                    'error'     => $e->getMessage(),
                ]);
            } finally {
    ...
    {
        return array_merge(['cards', 'billing', 'cancel'], $this->tenantTags());
    }
}
