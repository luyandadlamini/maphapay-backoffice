...
final class CashInController extends Controller
{
    private const PROVIDER_ID = 'standard_unayo';
    ...
    public function create(Request $request, MockWalletStore $store): JsonResponse
    {
        ...
        $payer = (string) $request->input('payer.msisdn', '');
        $amountMinor = MoneyConverter::toSmallestUnit($amount, 2);

        $sync = MockFailureRules::syncOutcome($payer, $amountMinor);
    ...
    public function show(string $ref, MockWalletStore $store): JsonResponse
    {
        ...
        $body = [
            ...
            'payer'        => ['msisdn' => (string) ($req['account_ref'] ?? '')],
