...
final class CollectionController extends Controller
{
    public function create(Request $request, MockWalletStore $store): Response|JsonResponse
    {
        ...
        $payerMsisdn = (string) $request->input('payer.partyId', '');
        $amountMinor = MoneyConverter::toSmallestUnit($amount, 2);

        $syncOutcome = MockFailureRules::syncOutcome($payerMsisdn, $amountMinor);
        ...
        $store->putRequest('mtn_momo', 'collect', $referenceId, [
            'account_ref'   => $payerMsisdn,
            'amount'        => $amount,
        ...
        DispatchMockWalletCallbackJob::dispatch('mtn_momo', 'collect', $referenceId)
    ...
    {
        $request = $store->getRequest('mtn_momo', 'collect', $ref);
        if ($request === null) {
