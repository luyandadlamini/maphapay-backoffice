...
class MigrateLegacyBalances extends Command
{
    ...
    private const SNAPSHOT_RUN_TYPE = 'balance_migration';

    private const SNAPSHOT_KEY = 'snapshot_taken_at';
    ...
    public function handle(): int
    {
        ...
        $rawThreshold = (string) $this->option('threshold');
        $this->threshold = is_numeric($rawThreshold) && (float) $rawThreshold >= 0 ? $rawThreshold : '0.01';

        if ($this->option('snapshot')) {
            $this->snapshotTime = now();
            $this->info('[snapshot] New snapshot timestamp: ' . $this->snapshotTime->toIso8601String());
            if (! $this->dryRun) {
                $this->saveSnapshotTakenAt($this->snapshotTime);
            }
        } else {
            $existing = $this->getSnapshotTakenAt();
            if ($existing) {
                $this->snapshotTime = $existing;
        ...
        if (! $this->option('force') && ! $this->confirm('Have you verified all prerequisites above?')) {
            $this->info('Aborted.');
        ...
        $legacyUsers = DB::connection('legacy')
            ...
            ->chunk($this->chunkSize, function ($rows) use (
                ...
                foreach ($rows as $row) {
                    ...
                    $legacyBalanceMajor = number_format((float) $row->balance, 8, '.', '');
                    ...
                    $this->line(sprintf(
                        '  [%s] legacy=%s snapshot=%s delta=%s target=%s (diff=%s)',
                        $row->id,
                        $legacyBalanceMajor,
                        $snapshotBalanceMajor,
                        $deltaAmount,
                        $targetBalanceMajor,
                        $diff,
                    ));
    ...
    private function getSnapshotBalance(int $legacyUserId, string $finaegisUuid): string
    {
        ...
        if ($row) {
            return number_format((float) $row->balance_major, 8, '.', '');
        }

        return '0.00000000';
    }

    private function getDeltaAmount(int $legacyUserId, ?string $legacyUuid): string
    {
        if ($legacyUuid === null) {
            return '0.00000000';
        }

        $credits = (string) DB::connection('legacy')
        ...
        $debits = (string) DB::connection('legacy')
            ->table('migration_delta_log')
            ->where('legacy_user_id', $legacyUserId)
        ...
        return bcsub(
            number_format((float) $credits, 8, '.', ''),
            number_format((float) $debits, 8, '.', ''),
            8,
        );
    }

    private function getSnapshotTakenAt(): ?Carbon
    {
        $row = DB::table('migration_run_logs')
            ->where('run_type', self::SNAPSHOT_RUN_TYPE)
            ->where('key', self::SNAPSHOT_KEY)
            ->first();

        return $row ? Carbon::parse($row->value) : null;
    }

    private function saveSnapshotTakenAt(Carbon $time): void
    {
        DB::table('migration_run_logs')->upsert(
            [
                'run_type'   => self::SNAPSHOT_RUN_TYPE,
                'key'        => self::SNAPSHOT_KEY,
                'value'      => $time->toIso8601String(),
                'created_at' => now(),
                'updated_at' => now(),
            ],
            uniqueBy: ['run_type', 'key'],
